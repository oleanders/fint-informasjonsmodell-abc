...
def main():
    ...
    ap.add_argument('--include-felles', action='store_true', help='Also generate for felles packages (default: skip)')
    args = ap.parse_args()
    ...
    for pkg_path in pkg_paths:
        if is_meta(pkg_path):
            continue
        # skip felles unless explicitly included
        if not args.include_felles and pkg_path[1].lower() == 'felles':
            continue
        # file name and write
